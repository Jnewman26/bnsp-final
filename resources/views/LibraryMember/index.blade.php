@extends('layout.template-member')
...
@section('content')
    <div class="justify-content-between flex-wrap flex-md-nowrap align-items-center pt-4 pb-2 mb-3 border-bottom">
        ...
        <form action="{{ url('/discoveryMember') }}" method="GET">
        ...
        @foreach ($book as $index => $item)
            ...
            <div class="modal fade" id="bookDetailModal{{ $item->book_isbn }}" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <form action="{{ url('discoveryMember') }}" method="POST">
                        @csrf
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                @if (session('member_status') !== 0)
                                    <hr>
                                    <h4>Borrower data</h4>
                                    <div class="row">
                                        <input type="hidden" value="{{ session('member_id') }}" name="member_id">
                                        <div class="col-12">
                                            <label for="form-label">Qty <span class="text-danger">*</span></label>
                                            <input type="number" name="borrowing_qty" class="form-control mt-2"
                                                min="1" max="{{ $item->book_stock }}" value="1">
                                        </div>
                                    </div>
                                @else
                                    <div class="alert alert-danger mt-4">
                                        Your account is on hold, you cannot borrow books
                                    </div>
                                @endif
